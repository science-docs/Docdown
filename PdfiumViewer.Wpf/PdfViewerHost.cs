...
{
    public class PdfViewerHost : ContentControl
    {
        ...
        static PdfViewerHost()
        {
            var type = typeof(PdfViewerHost);
        ...
        {
            if (d is PdfViewerHost viewer) viewer.viewer.ShowToolbar = (bool)e.NewValue;
        }

        private static void ShowBookmarksChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            if (d is PdfViewerHost viewer) viewer.viewer.ShowBookmarks = (bool)e.NewValue;
        }

        private static void DocumentChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            if (e.NewValue != null && d is PdfViewerHost viewer) viewer.viewer.Document = (IPdfDocument)e.NewValue;
        }

        private static void ZoomModeChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            if (d is PdfViewerHost viewer) viewer.viewer.ZoomMode = (PdfViewerZoomMode)e.NewValue;
        }

        private static void BackgroundChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            if (d is PdfViewerHost viewer)
            {
                switch (e.NewValue)
                {
