...
namespace PandocMark.Parser
{
    internal static class BlockMethods
    {
        ...
        {
            if (!block.IsOpen)
                throw new PandocMarkException(string.Format(CultureInfo.InvariantCulture, "Attempted to add line '{0}' to closed container ({1}).", ln, block.Tag));
        ...
        private static int ParseListMarker(string ln, int pos, bool interruptsParagraph, out ListData data)
        {
            ...
            if (c == '+' || c == '•' || ((c == '*' || c == '-') && 0 == Scanner.ScanThematicBreak(ln, pos, len)))
            {
                pos++;
            ...
            else if (c >= '0' && c <= '9')
            {
                ...
                if (interruptsParagraph &&
                    (start != 1 || Scanner.ScanSpacechars(ln, pos + 1, ln.Length) == ln.Length - pos - 1))
                    return 0;
