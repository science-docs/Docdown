...
namespace PandocMark.Parser
{
    internal static class InlineMethods
    {
        ...
        /// </summary>
        /// <returns></returns>
        internal static Func<Subject, Inline>[] InitializeParsers(PandocMarkSettings settings)
        {
            var strikethroughTilde = 0 != (settings.AdditionalFeatures & PandocMarkAdditionalFeatures.StrikethroughTilde);
            var placeholderBracket = 0 != (settings.AdditionalFeatures & PandocMarkAdditionalFeatures.PlaceholderBracket);
        ...
        private static Reference LookupReference(DocumentData data, StringPart lab)
        {
            ...
            if (data.ReferenceMap.TryGetValue(label, out Reference r))
                return r;
...
#endif
        private static char PeekChar(Subject subj)
        {
        ...
        static Inline HandleBackticks(Subject subj)
        {
            int ticklength = 0;
            var bl = subj.Length;
        ...
        private static int ScanEmphasisDelimiters(Subject subj, char delimiter, out bool canOpen, out bool canClose)
        {
            ...
            Utilities.CheckUnicodeCategory(charBefore, out bool beforeIsSpace, out bool beforeIsPunctuation);
            Utilities.CheckUnicodeCategory(charAfter, out bool afterIsSpace, out bool afterIsPunctuation);
        ...
        internal static int MatchInlineStack(InlineStack opener, Subject subj, int closingDelimiterCount, InlineStack closer, InlineTag singleCharTag, InlineTag doubleCharTag)
        {
            ...
            if (closer != null)
            {
                ...
                if ((closer.DelimiterCount -= useDelims) > 0)
                {
                    ...
                    {
                        SourcePosition = inl.SourceLastPosition = clInl.SourcePosition + useDelims,
                        SourceLength = closer.DelimiterCount,
                        NextSibling = clInl.NextSibling
                    };
        ...
        private static Inline HandleOpenerCloser(Subject subj, InlineTag singleCharTag, InlineTag doubleCharTag)
        {
            ...
            var numdelims = ScanEmphasisDelimiters(subj, c, out bool canOpen, out bool canClose);
            ...
            if (canOpen || canClose)
            {
                var istack = new InlineStack
                {
                    ...
                    Priority = InlineStack.InlineStackPriority.Emphasis,
                    Flags = (canOpen ? InlineStack.InlineStackFlags.Opener : 0)
        ...
        private static Inline HandleLeftSquareBracket(Subject subj, bool isImage)
        {
            ...
            var istack = new InlineStack
            {
                ...
                Priority = InlineStack.InlineStackPriority.Links,
                Flags = InlineStack.InlineStackFlags.Opener | (isImage ? InlineStack.InlineStackFlags.ImageLink : InlineStack.InlineStackFlags.None)
            };
        ...
        private static Inline HandleRightSquareBracket(Subject subj, bool supportPlaceholderBrackets)
        {
            ...
            var istack = InlineStack.FindMatchingOpener(subj.LastPendingInline, InlineStack.InlineStackPriority.Links, '[', 1, false, out bool canClose);
        ...
        private static Inline HandleBackslash(Subject subj)
        {
            subj.Position++;
        ...
        /// <returns></returns>
        private static Inline HandleTex(Subject subj)
        {
            int start = subj.Position - 1;
            char nextChar;
            for (; subj.Position < subj.Buffer.Length; subj.Position++)
            {
                nextChar = subj.Buffer[subj.Position];
                if (!Utilities.IsAsciiLetterOrDigit(nextChar))
                {
                    break;
                }
            }
        ...
        private static string ParseEntity(Subject subj)
        {
            int match;
            var origPos = subj.Position;
            match = Scanner.ScanEntity(subj.Buffer, subj.Position, subj.Length - subj.Position, out string entity, out int numericEntity);
            if (match > 0)
            {
                subj.Position += match;
        ...
        public static string Unescape(string url)
        {
            ...
            while ((searchPos = url.IndexOfAny(search, searchPos)) != -1)
            {
                ...
                {
                    match = Scanner.ScanEntity(url, searchPos, url.Length - searchPos, out string namedEntity, out int numericEntity);
                    if (match == 0)
                    {
                        searchPos++;
                    }
        ...
        static Inline HandlePointybrace(Subject subj)
        {
            int matchlen;
            string contents;
            ...
            matchlen = Scanner.ScanAutolinkUri(subj.Buffer, subj.Position, subj.Length);
            if (matchlen > 0)
            {
            ...
            matchlen = Scanner.ScanAutolinkEmail(subj.Buffer, subj.Position, subj.Length);
            if (matchlen > 0)
            {
                contents = subj.Buffer.Substring(subj.Position, matchlen - 1);
                var resultContents = ParseStringEntities(contents);
            ...
            matchlen = Scanner.ScanHtmlTag(subj.Buffer, subj.Position, subj.Length);
            if (matchlen > 0)
            {
        ...
        static Reference ParseLinkDetails(Subject subj, bool supportPlaceholderBrackets)
        {
            ...
            if (c == '(' &&
                    ((sps = Scanner.ScanSpacechars(subj.Buffer, subj.Position + 1, subj.Length)) > -1) &&
                    ((n = Scanner.ScanLinkUrl(subj.Buffer, subj.Position + 1 + sps, subj.Length)) > -1))
            {
                // try to parse an explicit link:
                starturl = subj.Position + 1 + sps; // after (
                endurl = starturl + n;
                starttitle = endurl + Scanner.ScanSpacechars(subj.Buffer, endurl, subj.Length);
                // ensure there are spaces btw url and title
                endtitle = (starttitle == endurl) ? starttitle :
                           starttitle + Scanner.ScanLinkTitle(subj.Buffer, starttitle, subj.Length);
                endall = endtitle + Scanner.ScanSpacechars(subj.Buffer, endtitle, subj.Length);
        ...
        static Inline HandleNewline(Subject subj)
        {
        ...
        public static Inline ParseUInline(Subject subj, Func<Subject, Inline>[] parsers, char[] specialCharacters)
        {
            var len = subj.Length;
        ...
        public static int ParseReference(Subject subj)
        {
            ...
            matchlen = Scanner.ScanLinkTitle(subj.Buffer, subj.Position, subj.Length);
            if (matchlen > 0)
            {
            ...
            while ((c = PeekChar(subj)) == ' ') subj.Position++;
